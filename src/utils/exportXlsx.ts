...
export async function exportRows(filename: string, sheetName: string, rows: XlsxRow[]) {
  if (!rows || !rows.length) {
...
export async function exportSheets(filename: string, sheets: { name: string; rows: XlsxRow[] }[]) {
  const XLSX = await import("xlsx");
  const wb = XLSX.utils.book_new();
  let any = false;
